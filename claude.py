# =============================================================================
# SOFTWARE DI ANALISI STATISTICA INCIDENTI STRADALI (v4.3 - Corretto)
# =============================================================================
import tkinter
...
class App(customtkinter.CTk):
    ...
    def esegui_analisi_descrittiva(self, *args):
        ...
        if variable == 'Data_Ora_Incidente': self.analisi_speciale_data_ora()
        else: self.analisi_generica(variable)
    ...
    def analisi_speciale_data_ora(self):
        ...
        info = "Analisi multi-livello della variabile temporale per identificare pattern orari, settimanali e andamenti di lungo periodo."
        guida = "- **Distribuzione Oraria:** Identifica le ore del giorno più a rischio.\n- **Distribuzione Settimanale:** Mostra i picchi nei giorni feriali/festivi.\n- **Andamento Temporale:** Rivela trend stagionali o variazioni."
        self._crea_titolo_sezione(self.frame_risultati_descrittiva, 0, "Analisi Temporale Dettagliata", info, testo_guida=guida)
        
        # CORREZIONE: Usare .grid() invece di .pack()
        frame_grafici = customtkinter.CTkFrame(self.frame_risultati_descrittiva)
        frame_grafici.grid(row=1, column=0, sticky='nsew', padx=5, pady=5)
        self.frame_risultati_descrittiva.grid_rowconfigure(1, weight=1) # Permette al frame di espandersi
        
        frame_grafici.grid_columnconfigure((0, 1), weight=1)
        frame_grafici.grid_rowconfigure(1, weight=1)

        days_map = {0: 'Lunedì', 1: 'Martedì', 2: 'Mercoledì', 3: 'Giovedì', 4: 'Venerdì', 5: 'Sabato', 6: 'Domenica'}
        days_order = list(days_map.values())

        fig1, ax1 = plt.subplots(figsize=(6, 4)); hourly_counts = self.df['Ora'].value_counts().sort_index(); hourly_counts.plot(kind='bar', ax=ax1, color='skyblue'); ax1.set_title('Distribuzione Incidenti per Ora'); ax1.set_xlabel('Ora del Giorno'); ax1.set_ylabel('Numero di Incidenti'); ax1.grid(axis='y', linestyle='--', alpha=0.7); fig1.tight_layout()
        canvas1 = FigureCanvasTkAgg(fig1, master=frame_grafici)
        canvas1.get_tk_widget().grid(row=0, column=0, padx=5, pady=5, sticky="nsew")
        self.matplotlib_widgets.append(canvas1)

        fig2, ax2 = plt.subplots(figsize=(6, 4)); daily_names = self.df['Data_Ora_Incidente'].dt.weekday.map(days_map); daily_counts = daily_names.value_counts().reindex(days_order)
        daily_counts.plot(kind='bar', ax=ax2, color='salmon'); ax2.set_title('Distribuzione Incidenti per Giorno'); ax2.set_xlabel('Giorno della Settimana'); ax2.set_ylabel('Numero di Incidenti'); ax2.tick_params(axis='x', rotation=45); ax2.grid(axis='y', linestyle='--', alpha=0.7); fig2.tight_layout()
        canvas2 = FigureCanvasTkAgg(fig2, master=frame_grafici)
        canvas2.get_tk_widget().grid(row=0, column=1, padx=5, pady=5, sticky="nsew")
        self.matplotlib_widgets.append(canvas2)

        fig3, ax3 = plt.subplots(figsize=(12, 4)); date_counts = self.df.groupby(self.df['Data_Ora_Incidente'].dt.date).size(); date_counts.plot(kind='line', ax=ax3, marker='.', linestyle='-', markersize=4); ax3.set_title('Andamento Temporale degli Incidenti'); ax3.set_xlabel('Data'); ax3.set_ylabel('Numero di Incidenti'); ax3.grid(True, linestyle='--', alpha=0.7); fig3.tight_layout()
        canvas3 = FigureCanvasTkAgg(fig3, master=frame_grafici)
        canvas3.get_tk_widget().grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="nsew")
        self.matplotlib_widgets.append(canvas3)
        plt.close('all')
    ...
    def analisi_generica(self, variable):
        ...
        try:
            ...
            if tipo_grafico == 'Istogramma':
                ...
                if tipo_grafico == 'Barre': freq_data.plot(kind='bar', ax=ax)
                elif tipo_grafico == 'Linee': freq_data.sort_index().plot(kind='line', ax=ax, marker='o')
                elif tipo_grafico == 'Torta': freq_data.plot(kind='pie', ax=ax, autopct=lambda p: f'{p:.1f}%' if p > 3 else ''); ax.set_ylabel('')
                elif tipo_grafico == 'Aste': ax.stem(freq_data.index.astype(str), freq_data.values)
                # CORREZIONE: Rimosso ha="right"
            ...
            canvas.get_tk_widget().pack(fill='both', expand=True) # .pack() qui va bene perché frame_grafico è un contenitore "privato"
    ...
    def esegui_analisi_bivariata(self, *args):
        ...
        try:
            ...
            if var_x == var_y:
                correlation, slope, intercept = 1.0, 1.0, 0.0
            else:
                regression = stats.linregress(x=x_data, y=y_data)
                slope, intercept, r_value = regression.slope, regression.intercept, regression.rvalue
                correlation = r_value

            frame_testuali = customtkinter.CTkFrame(self.frame_risultati_bivariata)
            frame_testuali.grid(row=1, column=0, sticky="ew", padx=10)
            risultati = f"Coefficiente di Correlazione (r): {correlation:.4f}\nEquazione Retta: y = {slope:.4f}x + {intercept:.4f}"
        ...
        try:
            ...
            if len(incidenti_giorno) < 2:
                ...
                if n == 0 or np.isnan(std) or std == 0:
                    risultato = "Impossibile calcolare: deviazione standard è zero."
                else:
                    ...
                    risultato = (f"STIMA INCIDENTI GIORNALIERI MEDI - {provincia.upper()}\n"
                                 ...
                                 f"Siamo sicuri al {livello}% che la vera media giornaliera sia in questo intervallo.")
        except Exception as e:
